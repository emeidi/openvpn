...
CONFIG="/usr/local/apps/openvpn/OPENVPN47/OPENVPN47.conf"
...
if [ ! -e "$IP" ]
then
	echo "Could not find iptable binary ($IP). Aborting."
	exit 1
fi
...
echo "Adding route to Melchtalstrasse ..."
CMD="route add -net 10.0.17.0 netmask 255.255.255.0 gw $ENDPOINT"
echo $CMD
eval $CMD
echo "Done."
echo ""
